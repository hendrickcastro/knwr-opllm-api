...
class HuggingFaceModel(IClient):
    ...
    def load(self) -> None:
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(self.modelName, token=self.hf_token)
            self.model = AutoModelForCausalLM.from_pretrained(self.modelName, token=self.hf_token)
            logger.info(f"Successfully loaded model and tokenizer for {self.modelName}")
        except Exception as e:
            logger.error(f"Error loading model {self.modelName}: {str(e)}")
            raise
    ...
    def generate_chat(self, messages: List[Dict[str, str]], max_tokens: Optional[int] = None, temperature: float = 0.7, **kwargs) -> Optional[object]:
        ...
        try:
            ...
            logger.error(f"Error generating chat with HuggingFace model {self.modelName}: {str(e)}")
            raise
    ...
    def generate(self, prompt: str, max_tokens: Optional[int] = None, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            logger.error(f"Error generating text with HuggingFace model {self.modelName}: {str(e)}")
            raise
    ...
    def get_info(self) -> Dict[str, Any]:
        return {"name": self.modelName, "type": "huggingface"}
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            logger.error(f"Error generating embedding with HuggingFace model {self.modelName}: {str(e)}")
            raise
